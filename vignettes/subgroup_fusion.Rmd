---
title: "Using the fuserplus package for prediction over subgroups"
author: "EngineerDanny"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Fuserplus Subgroup Prediction}
...
The `fuserplus` package implements l1 penalised regression with fusion over subgroups, as described in @dondelinger_16. The fusion penalties allow for information sharing across regression parameters for different subgroups in heterogeneous datasets.
...
Alternatively, we can use the compiled implementation of the proximal algorithm. This can be faster on some systems, especially for large numbers of subgroups.
...
Estimating the linear coefficients via an l2 fusion penalty is done via the wrapper function `fusedL2DescentGLMNet`, which internally performs the required transformation and calls `glmnet`.
