...
.genFusedLassoProximal <- function(X, Y, groups, group.names = sort(unique(groups)),
  ...
  if (c.flag) {
    ...
    # Call R function
    return(.genFusedLassoProximal.R(
      Y, groups, group.names, XX, XY, p, k, samp.sizes, lambda, gamma,
  ...
  function(X, Y, groups, lambda, gamma, G, mu = 1e-04, tol = 1e-06,
    ...
    return(
      .genFusedLassoProximal(
        ...
        p, k, samp.sizes, lambda, gamma, C, G,
        epsilon = mu * p * dim(C)[2],
